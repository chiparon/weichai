...
1. **伪相关反馈（PRF）—— 当前证据最强的一步**：`source-fragment` 通道已把正确的文件排到 rank 1，
   因此从**排名靠前的检索片段中挖掘标识符形状的词**、再回查声明通道，可以直接补上词表缺失的词（如 `copy`）。
   这是通用机制（离线、零网络、不依赖标注），且不针对任何单条题目。
   **注意**：直接往词表里补「整体写入→copy」会污染已冻结的 holdout（其头部明确禁止参与调参），因此不作为选项。
2. **重排层**：`java-content-type` 的现象说明同名不同类的区分仍然依赖打分，PRF 之后再评估。
3. **扩大标注集**：dev 已饱和（12/12）、holdout 仅 6 题。任何后续改动都必须在更大规模上验证，
   否则会重犯"对着无法区分的指标调参"的错误。
4. **嵌入模型升级**：`ts-stream-copy` 这类"中文语义→英文标识符"的失败正是跨模态通道的能力上限，
   接口已就绪，改配置 + 重建索引即可。
