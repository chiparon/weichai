...
在插件右上角打开 **设置 → DeepSeek API Key → 配置 API Key**，通过 VS Code 密码输入框录入。密钥保存在 VS Code SecretStorage，不进入工作区设置、Webview 或 Git；“更换 API Key”立即作用于后续请求。“清除保存的 Key”只删除插件凭据，后端已有 `.env` 配置仍可作为备用。
