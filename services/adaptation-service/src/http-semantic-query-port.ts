...
export interface HttpSemanticQueryPortOptions {
  /** Host-owned SemanticQueryPort endpoint, without a required trailing slash. */
  endpoint: string;
 ...
 * SeekDB connection: the VS Code host remains the sole semantic-index owner.
 */
  ...
  async #invoke<K extends keyof SemanticQueryPort>(
    ...
    if (!response.ok) {
      ...
      try {
        ...
        // A host error need not be JSON and should not change this boundary.
      }
