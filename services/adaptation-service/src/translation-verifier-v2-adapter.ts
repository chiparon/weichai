...
import { DEFAULT_VERIFICATION_STRATEGY, assertVerificationReceipt, type VerificationReceipt, type VerificationResult, type VerificationService, type VerificationStrategyDescriptor } from "@forexplore/translation-verifier";
...
export class TranslationVerifierV2Adapter implements MigrationBehaviorVerifierV2 {
  ...
  readonly strategyDescriptor: VerificationStrategyDescriptor = DEFAULT_VERIFICATION_STRATEGY;
  ...
  verifyWithReceipt(input: MigrationBehaviorVerificationInputV2, signal?: AbortSignal): Promise<VerificationReceipt> {
    // SAFETY: Analysis and plan contain JSON-compatible fields; their interfaces lack an index signature.
    ...
    return this.service.verifyWithReceipt(request, {}, signal).then((receipt) =>
      assertVerificationReceipt(receipt, request, this.strategyDescriptor));
  }
