...
function deterministicProviders() {
  const analyzer: MigrationAnalyzerV2 = {
    ...
    analyze: vi.fn(async (input) => {
      expect(input.sourceBundle.files[0]?.content).toBe(
        adaptationV2SourceContent,
      );
      ...
      return {
        ...
        mappings: [
          {
            source: "String.trim/toUpperCase",
            target: "str.strip/upper",
            rationale: "Equivalent string operations",
          },
        ],
        risks: [],
        unresolved: [],
...
describe("status-free adapter gate matrix", () => {
  it.each([3, 4])(
    "maps a real disk receipt cancelled at microtask %i to unverified",
    async (microtasks) => {
      ...
      const files: VerificationInput["translation"]["files"] = [
        {
          ...
          hunks: [
            {
              header: "@@ -0,0 +1,1 @@",
              lines: [{ type: "add", content: "value = 1" }],
            },
          ],
        },
      ];
  ...
  it.each([
    ...
    "maps %s",
    async (_name, executionStatus, sourceAssessment, targetAssessment, expected) => {
...
describe("AdaptationAdapterV2", () => {
    ...
    async (failure) => {
      ...
      try {
        const adapter = new AdaptationAdapterV2({
          runtimeCapabilities: fixture.serviceRuntime,
          ...providers,
          verifier: new TranslationVerifierV2Adapter(service),
        });
        if (failure === "signal-abort") {
          await expect(
            adapter.adapt(
          ...
          const behavior = result.validation.find(
  ...
  it("binds real artifacts from two repair rounds into a canonical manifest without ID collisions", async () => {
    ...
    const fixture = createAdaptationV2TestFixture();
    const providers = deterministicProviders();
    providers.translator.repair = vi.fn(async (_i, _a, _p, previous) => ({
      ...previous,
      ...
      factory: new VerificationStrategyFactory([
        {
          ...
          create: () => ({
            async verify(input, context) {
              writeFileSync(
                join(context.workspace.evidenceRoot, "report.json"),
                JSON.stringify({ round: input.translation.round }),
              );
              const artifact = await context.writeArtifact({
                id: "report",
                kind: "report",
    ...
    try {
      ...
      const artifacts = [
        ...result.validation.flatMap((record) =>
      ...
      for (const artifact of artifacts) {
        expect(
          createHash("sha256")
            .update(readFileSync(join(artifactRoot, artifact.path)))
            .digest("hex"),
        ).toBe(artifact.contentHash);
      }
      const route = fixture.validationContext.runtimeCapabilities.routes.find(
        (route) => route.id === result.route.routeId,
      )!;
  ...
  it("sends structured repair feedback without strategy reports", async () => {
    ...
    const translator = new DeepSeekMigrationTranslatorV2({
      apiKey: "test-key",
    ...
    const plan = {
      ...
      steps: [],
      preservedFacts: [],
      expectedTargetChanges: [],
      validationFocus: [],
      unresolved: [],
    };
    await translator.repair(
      fixture.request as never,
      analysis,
      plan,
      {
        schemaVersion: "1.0",
        generatedContent: "previous",
        completedSteps: [],
        unresolved: [],
      },
      {
    ...
    expect(payload.feedback.issues).toEqual([
      expect.objectContaining({ kind: "behavioral-divergence" }),
    ]);
  ...
  it("uses the full TypeScript bundle and adapter-owned Python facts to produce a validated patch", async () => {
    ...
    const fullFileFact = fixture.targetContext.sourceFiles.find(
      (fact) => fact.role === "source-file",
    );
    expect(fullFileFact?.contentHash).toBe(
      fixture.request.target.entity.fileContentHash,
    );
    ...
    expect(patchLines).toContainEqual({
      type: "add",
      content: "    return trimmed.upper()",
    });
    expect(patchLines).not.toContainEqual(
    ...
    expect(result.validation).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          policyCheckId: "patch-boundary",
          status: "pass",
        }),
        expect.objectContaining({
          policyCheckId: "target-compile",
          status: "pass",
        }),
      ]),
    );
  ...
  it("retains missing required verifier evidence as unverified and blocks the gate", async () => {
    ...
    expect(
      ...
      allowed: false,
      blockers: [
        expect.objectContaining({ policyCheckId: "behavior-differential" }),
      ],
    });
  });
  ...
  it.each<[string, VerificationResultMutation]>([
    [
      "stale subjectHash",
      (input) => ({
        ...validVerificationResult(input).result,
    ...
    [
      "wrong strategyId",
      (input) =>
        validVerificationResult(input, undefined, {
          ...behaviorStrategyDescriptor,
    ...
    "fails closed when the behavior verifier returns a %s",
    async (_name, mutate) => {
      const fixture = createAdaptationV2TestFixture();
      const verifier: MigrationBehaviorVerifierV2 = {
        providerId: "forexplore.translation-verifier.differential",
  ...
  it("runs one repair from behavior failure to pass and records normalized feedback", async () => {
    ...
    const providers = deterministicProviders();
    providers.translator.repair = vi.fn(async () => ({
      schemaVersion: "1.0",
      generatedContent: adaptationV2GeneratedContent + "\n# repaired",
      completedSteps: [],
      unresolved: [],
    ...
    expect(providers.translator.repair.mock.calls[0]?.[4].issues).toEqual([
      expect.objectContaining({ kind: "behavioral-divergence" }),
    ]);
    expect(result.repairRounds).toHaveLength(1);
    expect(
      result.validation.every(
        (record) => record.subjectHash === result.patchHash,
      ),
    ).toBe(true);
  });
  ...
  it("caps three behavior failures at two chained repairs", async () => {
    ...
    expect(result.repairRounds[1]!.inputPatchHash).toBe(
      result.repairRounds[0]!.outputPatchHash,
    );
    expect(
  ...
  it("does not repair when the compiler is unavailable", async () => {
    ...
    const result = await new AdaptationAdapterV2({
      ...
      compiler: {
        ...
        validate: () => ({ status: "unverified", summary: "unavailable" }),
      },
    }).adapt(fixture.request, fixture.validationContext);
    expect(providers.translator.repair).not.toHaveBeenCalled();
    expect(
      result.validation.find(
        (record) => record.policyCheckId === "target-compile",
      ),
  ...
  it("rejects an unchanged repair before another verification", async () => {
    ...
    await expect(
      new AdaptationAdapterV2({
        runtimeCapabilities: fixture.serviceRuntime,
        ...providers,
        verifier,
      }).adapt(fixture.request, fixture.validationContext),
    ).rejects.toThrow("new patch hash");
  ...
  it("repairs an available compiler failure with normalized feedback", async () => {
    const fixture = createAdaptationV2TestFixture();
    const route = fixture.serviceRuntime.routes.find(
      (candidate) => candidate.id === fixture.request.route.routeId,
    )!;
    const compilerStage = route.stages.find(
      (stage) => stage.stage === "compile-validation",
    )!;
    const compilerCalls: string[] = [];
    const callOrder: string[] = [];
    const compiler = {
      capability: () => ({
        providerId: compilerStage.providerId,
        providerVersion: compilerStage.providerVersion,
      }),
      validate: () => {
    ...
    const providers = deterministicProviders();
    providers.translator.repair = vi.fn(
      async (_input, _analysis, _plan, previous) => ({
        ...previous,
        generatedContent: previous.generatedContent + "\n# repaired",
      }),
    );
    ...
    expect(feedback.inputPatchHash).toBe(
      result.repairRounds[0]!.inputPatchHash,
    );
    ...
    expect(result.repairRounds[0]).toMatchObject({
      triggerValidationRecords: expect.arrayContaining([
        expect.objectContaining({
          policyCheckId: "target-compile",
          status: "fail",
        }),
  ...
  it("uses the linked custom compiler identity for repair feedback without fabricating compiler artifacts", async () => {
    ...
    const customize = (snapshot: typeof serviceBaseline) =>
      materializeMigrationRuntimeCapabilitySnapshot({
        createdAt: snapshot.createdAt,
        routes: snapshot.routes.map((route) => ({
          ...route,
          stages: route.stages.map((stage) =>
            stage.stage === "compile-validation"
              ? {
                  ...stage,
                  providerId: customProvider.providerId,
                  providerVersion: customProvider.providerVersion,
                }
              : stage,
          ),
          validationPolicy: {
            ...route.validationPolicy,
            checks: route.validationPolicy.checks.map((check) =>
              check.id === "target-compile"
                ? {
                    ...check,
                    id: "syntax-proof",
                    verifierId: customProvider.providerId,
                    verifierVersion: customProvider.providerVersion,
                  }
                : check,
            ),
          },
  ...
  it("passes exact round, hash, and content to every verifier attempt", async () => {
    ...
    expect(
      inputs.every((input) => input.files[0]?.hunks.length && input.patchHash),
    ).toBe(true);
    expect(result.repairRounds).toHaveLength(2);
  });
});
