...
### 14.3 候选上限标定：12 使 34 题全部命中

重排此前只覆盖融合排名的前 8 名，排在 8 名之外的目标提不上来——这是当时最后一个已知失败模式。
扫描 `RECAST_RETRIEVAL_RERANK_LIMIT`：

| 候选上限 | dev(12) | holdout(6) | 新集(16) | MRR（dev / holdout / 新集） |
| --- | --- | --- | --- | --- |
| 8 | 12/12 | 5/6 | 15/16 | 0.925 / 0.683 / 0.938 |
| **12（采用）** | **12/12** | **6/6** | **16/16** | **1.000 / 0.917 / 1.000** |
| 16 | 12/12 | 6/6 | 16/16 | 1.000 / 0.917 / 0.969 |

**上限 12 时三集合计 34/34 = 100% 命中**，nDCG 0.938–1.000，平均延迟 1.59–1.62 s、
p95 1.83–2.11 s（命题要求 ≤10 s）。上限 16 召回相同但新集 MRR 反降至 0.969，故取 12。

### 14.4 最终对照（无预算，官方口径）

| 配置 | 召回 | MRR（dev / holdout / 新集） |
| --- | --- | --- |
| 重排关闭（基线） | 32/34 = 94.1% | 0.656 / 0.193 / 0.518 |
| **重排开启（上限 12）** | **34/34 = 100%** | **1.000 / 0.917 / 1.000** |

### 14.5 已知局限

- 需要外部模型凭据与网络；默认关闭（`RECAST_RETRIEVAL_RERANK=on` 启用）。
- 候选上限（8/12/16）已扫描；预览字符数（240）与提示词结构未扫描。
- 重排路径为单次读数；关闭时基线逐位不变且 189/189 测试通过，但开启路径未做多轮重复验证。
