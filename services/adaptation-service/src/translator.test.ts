...
describe("Translator Agent", () => {
  ...
  it("stops for explicit Analyzer blocking issues", async () => {
    const request = fixture("translator-adapt");
    request.analysisReport.blockingIssues = ["unknown-library has no target equivalent"];
  ...
  it("keeps ordinary unresolved analysis as translator context", async () => {
    const request = fixture("translator-adapt");
    request.analysisReport.unresolved.push("Confirm gateway timeout ownership.");

    await expect(
      translateWithAnalysis(request, {
        apiKey: "test-key",
        request: modelRequest(resultFor(request)),
      }),
