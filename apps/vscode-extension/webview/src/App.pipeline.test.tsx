...
import { emptyTargetWorkspace } from '../../src/module-explorer';
...
it('opens without a method, saves two history paths, displays durable summaries and switches project scope', async () => {
  ...
  const emit = async () => {
    ...
    post({ type: 'PROJECT_EXPLORER', explorer: (await buildProjectExplorer(host)).presentation });
  };
  const reviewedExplorer = {
    generatedAt: '2026-09-01T00:00:00.000Z',
    target: emptyTargetWorkspace('等待已审目标目录', paths[2]!, false),
    history: [],
  };
  const sent: WebviewToHostMessage[] = [];
  const pending: Promise<void>[] = [];
  let pickedPathCount = 0;
  window.acquireVsCodeApi = () => ({ getState: () => null, setState: () => {}, postMessage: (raw) => {
    ...
    if (message.type === 'PICK_REPOSITORY_PATH') {
      post({ type: 'REPOSITORY_PATH_PICKED', path: paths[pickedPathCount++]! });
    }
  ...
  await act(async () => post({ type: 'INIT', payload: {
    workspaceRoot: paths[2]!, settings: { repositoryPaths: [], topK: 4 }, repositoryStatuses: [],
    codeIntelligence: await host.presentation(), moduleExplorer: reviewedExplorer,
    projectExplorer: (await buildProjectExplorer(host)).presentation,
  ...
  expect(container.textContent).toContain('项目摘要仅用于代码理解');
  ...
  await clickText('添加路径');
  expect(sent.filter((message) => message.type === 'PICK_REPOSITORY_PATH')).toHaveLength(2);
  ...
  expect(container.textContent).not.toContain('WRONG DISK SUMMARY');
  await act(async () => {
    post({ type: 'MODULE_EXPLORER', explorer: reviewedExplorer });
  });
  expect(container.textContent).toContain('Stored summary: history-b');
  await clickText('已审迁移目录');
  await act(async () => {
    container.querySelector<HTMLButtonElement>('.workspace-switch button:first-child')!.click();
  });
  expect(container.textContent).toContain('等待已审目标目录');
  expect(container.textContent).toContain('目标目录尚未可选');
  expect(container.textContent).not.toContain('Stored summary:');
  expect(sent.some((message) => ['SELECT_TARGET_ENTITY', 'START_TARGET_TRANSLATION', 'START_SEARCH', 'START_ADAPT'].includes(message.type))).toBe(false);
  await host.synchronize({ repositories: inputs() }); await host.waitForProjects();
  expect(plan).toHaveBeenCalledTimes(3);
}, 20_000);
